...
import {
  ...
  Grid3x3, Settings, ClipboardCheck, LogOut, DollarSign,
  FileText, CreditCard, UserPlus,
} from 'lucide-react';
...
// Navegación completa — admin y recepcionista
...
// Solo lo que necesita el profesor
...
function NavLink({ item, pathname }: { item: NavItem; pathname: string }) {
  ...
  return (
    ...
    >
      <item.Icon size={15} strokeWidth={active ? 2.25 : 1.75} className="shrink-0" />
      {item.label}
    </Link>
  );
}
    ...
    <>
      ...
      >
        ...
        <div className="flex h-[60px] shrink-0 items-center gap-3 px-5"
          ...
          <span className="select-none text-[13px] font-black tracking-[0.16em]"
        ...
        {role === 'admin' && (
          <div className="px-3 pb-1">
            {[
              ...
              <NavLink key={item.href} item={{ ...item, shortLabel: item.label }} pathname={pathname} />
            ))}
          </div>
        )}
              ...
              return (
                <li key={href} className="flex flex-1 items-center justify-center">
                  <Link href={href}
                    ...
                    style={{ background: active ? 'var(--nexa-text)' : 'var(--nexa-card-alt)', color: active ? '#FFFFFF' : 'var(--nexa-text-sub)' }}>
